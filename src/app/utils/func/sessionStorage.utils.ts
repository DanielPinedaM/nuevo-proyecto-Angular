...
export const convertToStringify = (
  value: TSessionStorageValue
): TSessionStorageValue => {
...
export const sessionStorageListAll = (): ISessionStorageObject | null => {
  ...
  for (let i: number = 0; i < length; i++) {
    ...
    if (key) {
      const value: TSessionStorageListValue = sessionStorageListValue(
        atob(key)
      );
...
export const sessionStorageListValue = (
  property: string
): string | any[] | any => {
...
export const sessionStorageUpdate = (
  property: string,
  value: TSessionStorageValue
): boolean => {
  if (!isValidString(property)) {
